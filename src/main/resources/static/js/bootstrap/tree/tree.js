$(function () {
    $.fn.bootstrapTree = function(options){
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            $(menu_li).append("<a href='javascript:void(0);' class='collapsed' expanded='false' data-toggle='collapse' data-target='#"+data[i].code+"'><i class='"+data[i].iCon+"'></i> "+data[i].name+" <i class='fa fa-fw fa-caret-down'></i></a>")
            if(data[i].child.length>0){
                // 添加二级节点并设置二级节点的html的属性
                var child = data[i].child;
                var child_menu_ul = document.createElement("ul");
                child_menu_ul.setAttribute("id", data[i].code);
                ...
                for(var j=0;j<child.length;j++){
                    ...
                    if(child[j].iCon!=''&&child[j].iCon!=undefined){
                        ...
                        $(child_menu_ul_li).find("a").bind("click",{node:child[j].node,name:child[j].name,url:child[j].url},function(e){
                            $("#"+opts.tabId).data("tabs").addTab({id: e.data.node, text: e.data.name, closeable: true, url: e.data.url});
                        })
                    }else{
